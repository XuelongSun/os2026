...
### 死锁DeadLock
其实，我们之前我们在讲用信号量解决生产者消费者问题时埋了一个伏笔，就是问互斥信号量和资源信号量的`wait`执行顺序能否交换：
```c
void* producer(void *args)
{
    while(1)
...
#### 死锁的避免
避免死锁的策略是：不提前采取限制措施，在资源的动态分配过程中防止系统进入不安全状态(可能发生死锁的状态)。

那么，何为安全？就是存在一个**安全序列**可是的每个进程顺利完成！此状态不会出现死锁，因为有一个可使所有进程顺利执行完成的CPU时序。

比如，P1,P2,P3三个进程，系统还有资源3个，如果目前：
...
如果现在P2申请2个单位的资源，分配给P2会使系统进入不安全状态吗？那么来分析若给P2两个单位的资源，是否存在一个安全序列使得3个进程都能完成。
...
sys --2--> P2 (sys=1)
        ↓
P2 finish, P2 --4--> sys (sys=5) 
        ↓
sys --5--> P1 (sys=0)
        ↓
P1 finish, P1 --10--> sys (sys=10)
        ↓
sys --7--> P3 (sys=3)
        ↓
P3 finish, P3 --9--> sys (sys=12)
```

但是，若不是P2请求2个，而是P3请求1个资源，该不该分配呢？若分配的话：
...
且系统只剩下2个，若分配给P2，那么等P2完成后，系统还剩4个，但是P1差5个，P3差6个，都无法执行，必须等对方先释放，这就造成了死锁，或者我们找不到一个安全序列！因此，P3请求1个资源时，应该拒绝！

我们把上述的3个进程，一种资源推广到：$N$ 个进程，共享 $M$ 种资源：
- 系统可用资源量为：$Ava_j, j=1,2...M$
- 每个进程的最大需求为：$M_{ij}, i=1,2..N, j=1,2,..M$
- 已分配给进程的资源为: $All_{i,j}, i=1,2..N, j=1,2,..M$
- 每个进程尚需的资源为: $N_{i,j}, i=1,2..N, j=1,2,..M$, 且：$N_{ij} = M_{i,j} - All_{ij}$

问：进程 $i$ 要申请：$P_R^i = [R_{i1}, R_{i2}, ...R_{iM}]$ 时，是否分配？

##### 银行家算法
解决上述问题，可用银行家算法Banker's Algorithm，该算法由Edsger Dijkstra于1968由保证银行有款课贷的思想提出。

该算法的流程图如下：

![](figures/banker_flowchart.png)

其核心思想就是检查针对每一个系统状态，尝试寻找安全序列。而寻找安全序列的思路，就是针对系统现有资源情况(work resource $W_j$)和进程需要($N_{ij}$)，找到能满足运行需要($N_{ij} \le W_j$)的进程先运行，运行完成后($F_i=true$)，归还资源，因而可以更新工作资源 $W_j:= W_j + All_{ij}$，再去寻找可运行的进程，直到遍历完所有进程。如果最后所有进程都能被这样消掉，系统就是安全的。

运行用Python实现的[银行家算法](source/deadlock/banker.py), 会得到如下输出：
```
=== Banker's Algorithm ===
Initial State Check: (True, ['P1', 'P3', 'P4', 'P0', 'P2'])
Process P1 request: [1, 0, 2]
safe: request granted
Safe sequence: P1 -> P3 -> P4 -> P0 -> P2
Process P4 request: [3, 3, 0]
No enough resources available, request denied
Process P0 request: [0, 1, 0]
safe: request granted
Safe sequence: P1 -> P3 -> P4 -> P0 -> P2
```
...
但是，认真分析这个代码你会发现一些问题：
1. 如何预知程序的资源需求？
2. 如何获得系统的最大可用资源量，这个资源是否也在不断变化
3. 操作系统种运行着诸多进程，每时每刻都要如此验证？

因此，这也不是现代操作系统解决死锁的办法。

#### 死锁的检测与解除
为了减少代价，我们可以先让程序运行，等到发生死锁了我们再想办法。那如何知道当前系统状态 $S$ 是否是死锁状态呢？这就需要进行死锁检测 Deadlock Detection。

一种常用的检测方法是基于资源分配图Resource Allocation Graph。

##### 资源分配图
定义资源分配图 $G = (N, E)$, 其中节点 $N$ 为进程节点 $\{P_0, P_1, P_2, ... P_n\}$ 和 资源节点 $\{R_0, R_1, R_2, ... R_m\}$(在资源节点中可用小点表示该资源的实例个数)，而边 $E$ 为请求边 ($P \to R$) 和分配边 ($R \to P$)。可以对资源分配图其进行化简：从某个进程节点开始，若现有资源可以满足其资源请求，则消去其所有边。若最后可以消去所有边，则称该图为完全可简化图。**死锁定理表明：死锁状态的充要条件是，当且仅当状态的资源分配图是不可完全简化图。**

例如：某系统某时刻有连个进程P1,P2在运行，系统资源R1共6个单位，已分配给P1两个单位，P2一个单位，系统资源R2共4两个单位，已分配给P2一个单位，分配给P1一个单位，此时，P1申请一个单位R2，P2申请一个单位R1。问：系统状态是否安全？

我们可以先做出资源分配图，再化简：

![](figures/resource_allocation_graph.png)

发现，可以完全化简。因此可知系统状态是安全的。

##### 死锁解除
若检测出系统处在发生死锁的不安全状态，则可采用以下方式解除死锁：
1. 抢占资源给死锁进程，使其可以继续运行。
2. 杀死死锁进程。

针对1，有些资源如果半途抢占将会形成破环或变得混乱无意义，比如打印到一半的打印机，写到一半的文件。

针对2，那么有两种方式：
（1）杀死所有进程：简单但代价太大(重要服务，关键文件读写，已运行了很久)
（2）逐个终止，直到死锁解除

那如何逐个终止呢？先终止哪个进程更好呢？一般我们考虑终止进程的代价：
- 优先级
- 已执行时间
- 资源使用和尚缺？
- 进程性质是交互式/批处理？

考虑上述因素，可构造代价函数 $C_i = f(p_i)$， 终止进程 $p_i$ 后系统由状态 $S \to U_i$, 因此要终止的进程为：
$$
\argmin_iC_i
$$

解得为$i^*$, 但若此时 $U_{i^*}$ 状态仍然是死锁状态，就需继续按上述方法选择进程然后终止进程。若计算一次代价函数的开销为$C$，假设系统终止 $k$ 个进程后达到安全状态，则总计算开销为：$n(n-1)...(n-k)C$。

可见，死锁检测和解除的开销也是很大的，那么我们现代操作系统是如何处理死锁问题的？

##### 鸵鸟算法
答案就是做一只鸵鸟，不处理，因为：
- 死锁较罕见（很多情况是用户程序的BUG）
- 避免、预防死锁代价都很大，且对计算机易用性有影响
- 若出现，则人工介入，杀死进程/重启系统

这就是**鸵鸟算法-Ostrich Algorithm**。

不过，对于系统内核等关键位置，现代操作系统会有一些机制处理死锁问题，比如Linux的`lockdep`--运行时锁依赖正确性验证器：它记录锁类别以及实际观察到的**锁获取依赖**。所以它主要是开发和调试阶段的运行时锁依赖验证工具，主要针对应用程序的并发开发这一主要引起进程死锁的诱因。

针对死锁的问题，我们总结如下：

![](figures/deadlock-summary.png)
---

到此，本章就结束了。我们从程序，执行流，线程讲到进程，来明确操作系统的管理对象。

然后我们讲了操作系统进程管理的设计原则：**默认隔离，显式合作**，然后由合作中的共享内存引出了互斥、同步与死锁的问题。

我们始终围绕操作系统让计算机更美好这一主题，以单核分时操作系统为讨论对象，揭示了进程管理中诸多问题和概念的工程必然性。

下一章，我们将继续探讨和进程有关的问题，主要涉及操作系统对进程的调度和操作系统对进程控制的基本实现。
