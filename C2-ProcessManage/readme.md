...
其核心思想就是检查针对每一个系统状态，尝试寻找安全序列。而寻找安全序列的思路，就是针对系统现有资源情况(work resource $W_j$)和进程需要($N_{ij}$)，找到能满足运行需要($N_{ij} \le W_j$)的进程先运行，运行完成后($F_i=true$)，释放资源，因而可以更新工作资源 $W_j:= W_j + All_{ij}$，再去寻找可运行的进程，直到遍历完所有进程。
