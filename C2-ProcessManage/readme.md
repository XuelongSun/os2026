...
简单思考一下，也知道，进程之间必须隔离。不然你在Offic Word中就可以操作浏览器了:smile: ！并且，一个进程出错了，不会影响其他进行，更不会影响你的计算机。
...
因为，在操作系统的加持下：运行的是同一个程序，却产生了相互独立的不同进程。这再一次体现出了程序只是静态的文件，而进程是动态的"运行世界"！而这个运行世界，包含了如下内容：
...
### 并发 Concurrency
理解了进程之后，我们知道，分时操作系统允许用户同时允许多个进程。
