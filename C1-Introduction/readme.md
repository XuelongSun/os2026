...
### 什么是操作系统 - What
当我们谈到操作系统时，你想到的可能是：Windows, MacOS, Linux等等。你可能想到的是自己曾经安装系统和操作电脑时的场景，但你真的感受过操作系统的存在吗？你想象过如何没有操作系统你的计算机会怎么样吗？另外，你真的清楚计算机开机后的流程吗？
...
那为什么向`fd=1`写就可以在终端显示字符串了呢？因为这个`fd=1`真正指向的文件是`tty`([终端模拟器](https://zhuanlan.zhihu.com/p/447014333))：
```bash
tty
-> /dev/pts/4
readlink /proc/$$/fd/1
-> /dev/pts/4
```
终端模拟器再把输入的字符串通过显卡控制器渲染在屏幕上，我们就可以看见了。
...
在`./hello_c`和`./hello_asm`真正启动运行之前：
```
谁读取 ELF？
谁建立地址空间？
谁建立 stack？
...
正因为操作系统以实现这些功能为其主要设计目的，这也就从根本上决定了操作系统的特点：
- 并发性：分时系统，多个程序并发运行
- 共享性：共享CPU，共享内存，共享IO
- 虚拟性
  - 时间：每个程序似乎独占CPU时间
  - 空间：每个程序似乎拥有整片内存
  - 外设：用户程序感觉所有外设操作起来几乎一样(设备无关性)
- 异步性：程序之间的交互，I/O交互

所以下一次提到操作系统，你的心中要有这些关键词！

学习操作系统并不是一件简单的事情，我们要用一个学期来回答操作系统究竟是什么，并不断体会操作系统如何让计算机更美好的！

这一路上，你也会不断看到，聪明的人类是如何解决操作系统中一个又一个的关键问题，从而能让今天的我们如此方便地使用计算机！
